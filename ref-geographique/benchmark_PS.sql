...
use database REF_DEV;
use schema REF_GEOGRAPHIQUE_SAS;
...
select CASE WHEN DL.SOVEREIGN IS NULL OR TRIM(DL.SOVEREIGN)='' THEN UPPER(PL.ISO)
       ...
       REPLACE(PC.POSTCODE,' ') AS NEUTRAL_POSTCODE,
       PL.REGION_ID,
       CASE WHEN COUNTRY_CODE = 'FR' AND DL.LANG1 = COUNTRY_CODE THEN RG.NAME_LANG1
            WHEN COUNTRY_CODE = 'FR' AND DL.LANG2 = COUNTRY_CODE THEN RG.NAME_LANG2
            WHEN COUNTRY_CODE = 'FR' AND DL.LANG3 = COUNTRY_CODE THEN RG.NAME_LANG3
            WHEN COUNTRY_CODE <> 'FR' AND DL.LANG1 = 'EN' THEN RG.NAME_LANG1
            WHEN COUNTRY_CODE <> 'FR' AND DL.LANG2 = 'EN' THEN RG.NAME_LANG2
            WHEN COUNTRY_CODE <> 'FR' AND DL.LANG3 = 'EN' THEN RG.NAME_LANG3
            ELSE RG.NAME_LANG1
       END AS REGION_NAME 
from REF_DEV.REF_GEOGRAPHIQUE_SAS.PLACES PL
     left outer join REF_DEV.REF_GEOGRAPHIQUE_SAS.REGIONS RG on RG.ID = PL.REGION_ID
     inner join REF_DEV.REF_GEOGRAPHIQUE_SAS.DATA_LANGUAGES DL on DL.ISO = PL.ISO
     inner join REF_DEV.REF_GEOGRAPHIQUE_SAS.PLACES_POSTCODES PPC on PPC.ISO = PL.ISO and PPC.PLACE_ID = PL.ID 
     inner join REF_DEV.REF_GEOGRAPHIQUE_SAS.POSTCODES PC on PC.ISO = PPC.ISO and PC.ID = PPC.POSTCODE_ID
...
call REF_DEV.PUBLIC.GET_POSTCODES_FROM_COUNTRY_AND_PLACE('FR',500054); -- Chicago



CREATE OR REPLACE TRANSIENT TABLE REF_DEV.PUBLIC.COUNTRY_POSTCODE_PLACES_SEARCH AS
select CASE WHEN DL.SOVEREIGN IS NULL OR TRIM(DL.SOVEREIGN)='' THEN UPPER(PL.ISO)
              ELSE UPPER(DL.SOVEREIGN) 
       END COUNTRY_CODE,
       PL.ID as PLACE_ID, PL.NAME_LANG1 as PLACE_NAME,
...
from REF_DEV.REF_GEOGRAPHIQUE_SAS.PLACES PL
     ...
     inner join REF_DEV.REF_GEOGRAPHIQUE_SAS.POSTCODES PC on PC.ISO = PPC.ISO and PC.ID = PPC.POSTCODE_ID
where PPC.IS_POSTAL = 1  
order by COUNTRY_CODE, nvl(NEUTRAL_POSTCODE,''), LOCALITY 
;


select count(1) as nb from REF_DEV.PUBLIC.COUNTRY_POSTCODE_PLACES_SEARCH;
-- 11 504 639 rows

SELECT * FROM REF_DEV.PUBLIC.COUNTRY_POSTCODE_PLACES_SEARCH
WHERE COUNTRY_CODE = 'FR'
  AND NEUTRAL_POSTCODE LIKE REF_DEV.PUBLIC.REMOVE_SPECIAL_CHARS_PLUS('820')||'%'
;

select  
   ISO||'_'||UPPER(NAME_EN) as SEARH_KEY
  ,ISO as COUNTRY_CODE
...
CREATE OR REPLACE TRANSIENT TABLE REF_DEV.PUBLIC.COUNTRY_PLACES_POSTCODES_SEARCH AS
WITH seldata as (
  select CASE WHEN DL.SOVEREIGN IS NULL OR TRIM(DL.SOVEREIGN)='' THEN UPPER(PL.ISO)
                ELSE UPPER(DL.SOVEREIGN) 
        END COUNTRY_CODE,
...
select sd.*,
       DL2.name_en as COUNTRY_NAME
from seldata sd
     inner join REF_DEV.REF_GEOGRAPHIQUE_SAS.DATA_LANGUAGES DL2 on DL2.ISO = sd.COUNTRY_CODE  
order by sd.COUNTRY_CODE, nvl(sd.NEUTRAL_LOCALITY,''), nvl(sd.NEUTRAL_POSTCODE,'');

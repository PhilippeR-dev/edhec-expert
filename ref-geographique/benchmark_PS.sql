...
DECLARE
    query VARCHAR DEFAULT 'SELECT PLACES_ID, POSTCODE, LOCALITY 
                           FROM REF_DEV.PUBLIC.PLACES_POSTCODES_SEARCH
                           WHERE SEARCH_KEY LIKE UPPER('''||:COUNTRY||''')||''_''||UPPER(REGEXP_REPLACE(trim(REF_DEV.PUBLIC.REMOVE_SPECIAL_CHARS('''||:INPUT_PLACE||''')), ''\\s+'', '' '', 1, 0))||''%'' 
                           LIMIT 50';
    res RESULTSET;
BEGIN
    res := (EXECUTE IMMEDIATE :query);             
...
call REF_DEV.PUBLIC.GET_ZIP_AND_PLACE_FROM_COUNTRY_AND_PLACE_SEARCH('US','Chic');
-- 2.8s --> Attention: déterioration de la performance
